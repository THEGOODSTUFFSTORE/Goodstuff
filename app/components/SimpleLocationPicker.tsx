...
export default function SimpleLocationPicker({
  ...
  return (
    <div className="space-y-4">
      ...
      <div className={`rounded-md p-3 ${deliveryFee === 0 ? 'bg-green-50 border border-green-200' : 'bg-yellow-50 border border-yellow-200'}`}>
        <p className={`text-sm font-medium ${deliveryFee === 0 ? 'text-green-800' : 'text-yellow-800'}`}>Delivery Fee:</p>
        <p className={`text-sm ${deliveryFee === 0 ? 'text-green-700' : 'text-yellow-700'}`}>
          {deliveryFee === 0 ? 'Free delivery within 10km' : `Ksh ${deliveryFee.toLocaleString()}`}
        </p>
      </div>
    </div>
  );
